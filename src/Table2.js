...
const Example = () => {
    ...
    const handleSelectAll = () => {
        if (selectedRows.length === tableData.length) {
            ...
            setSelectedRows(tableData.map((row) => row.user_slug));
        }
    };
    ...
    return (
        <TableContainer>
            ...
            <Table>
                ...
                <tbody>
                    {tableData.map((row) => (
                        <React.Fragment key={row.user_slug}>
                            <TableRow>
                                <TableCell>
                                    <TableCheckbox
                                        type="checkbox"
                                        checked={isRowSelected(row.user_slug)}
                                        onChange={() => handleSelectRow(row.user_slug)}
                                    />
                                </TableCell>
                                ...
                                <TableCell>
                                    <ExpandCollapseButton onClick={() => handleExpandCollapse(row.user_slug)}>
                                        {/* You can use icons like ▶ and ▼ for better UX */}
                                        {isRowSelected(row.user_slug) ? '▼' : '▶'}
